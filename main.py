...
class TelegramAutomationBot:
    ...
    def _load_config(self):
        ...
        try:
            if os.path.exists(CONFIG_FILE) and os.path.getsize(CONFIG_FILE) > 0:
                ...
                default_config = {
                    "telegram": {"session": "", "api_id": API_ID, "api_hash": API_HASH}, 
                    "bot": {"token": "", "chat_id": ""}
                }
                with open(CONFIG_FILE, "w") as f:
    ...
    def _save_config(self):
        ...
        try:
            ...
            # Try with more permissions or different location if needed
    ...
    async def login(self, phone=None, code=None, password=None):
        ...
        try:
            ...
            # Create client with session - using StringSession directly
    ...
    async def verify_code(self, phone, code):
        ...
        try:
            ...
            return False, f"Critical verification error: {str(e)}"
    ...
    async def verify_password(self, password):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error saving session: {str(save_err)}")
                return False, f"Login successful but error saving session: {str(save_err)}"
...
async def main():
    ...
    if not session or not await bot.client.is_user_authorized():
        ...
        # Get phone number with clear instructions
        print("\nPlease enter your phone number in international format.")
        print("Example: +12025550123 (include the '+' sign and country code)")
        ...
        if not success:
            ...
            print("\nPlease check Telegram on your mobile device for the verification code.")
            print("Enter the 5-digit code below:")
            ...
            if not success and "password" in message.lower():
                ...
                print("Please enter your account password (not the code):")
                ...
                print("\nCode verification failed. Please try running the script again.")
                return
    
    # Check if logged in successfully
    if not await bot.client.is_user_authorized():
        print("Login failed. Please try again by running the script again.")
        return
    
    print("\n✅ Successfully logged in!")
    
    # Join all groups/channels
    print("\nJoining groups/channels from your list...")
    try:
        joined, skipped, errors = await bot.join_all_groups()
        print(f"✅ Joined {joined} groups, skipped {skipped}, encountered {errors} errors")
    except Exception as e:
        print(f"Error joining groups: {str(e)}")
    
    # Check if bot token and chat ID are set
    if not bot_token:
        print("\nBot setup required.")
        print("You need to create a bot with @BotFather on Telegram and get the token.")
        print("Instructions: Message @BotFather on Telegram, use /newbot command and follow the steps.")
    ...
    if not chat_id:
        print("\nYou need to provide your personal Telegram chat ID.")
    ...
    if bot.start_bot():
        ...
        print("\nSend /start to your bot on Telegram to begin using it.")
